...
通过采样星体在一个周期内的位置，绘制星体的轨道。轨道的绘制采用`LineBasicMaterial`。为优化性能，采样点采用队列实现，无需每帧重新计算。需要指出，由于星体运动是基于真实的天文数据，不存在严格意义的周期性，因此绘制的轨道**不是完全闭合的**，这与实际情况一致。
...
本项目采用git进行版本控制。仓库地址为：[GitHub](https://github.com/hmk114/Stellara)。
分工如下：
